<template>
  <div class="my-10 mx-auto max-w-xl">
    <primary @click.native="$router.go(-1)" class="my-5">Go back</primary>
    <div class="blog" v-html="$md.render(content)"></div>
  </div>
</template>
...
export default {
  async asyncData({ $blogxon, route }) {
    let slug = route.params.slug
    let blog = await $blogxon.getBlogBySlug(slug)
